...
# Callback версії для inline кнопок
async def undo_last_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Callback версія скасування"""
    # Імітуємо звичайне повідомлення
    fake_message = type('obj', (object,), {'from_user': update.callback_query.from_user})()
    fake_update = type('obj', (object,), {'message': fake_message})()
    
    await undo_last_action(fake_update, context)
    
    # Оновлюємо повідомлення
    await update.callback_query.edit_message_text("✅ Дію виконано")
...
# ОРИГІНАЛЬНА ФУНКЦІЯ MAIN З ВАШОГО РОБОЧОГО КОДУ
async def main():
    """Запускає бота"""
    ...
    # ОРИГІНАЛЬНИЙ ЗАПУСК З ВАШОГО КОДУ
    try:
        ...
        try:
            ...
            logger.error(f"Помилка зупинки бота: {e}")(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Порівняння витрат між користувачами за місяць"""
    expenses = get_all_expenses()
    filtered_expenses = filter_expenses_by_period(expenses, "month")
